...
int main()
{
    ...
    int cropPadding = (4 - (cropWidthRGB & 3)) & 3; // the extra pad size
    fprintf(log, "Padding: %d\n", cropPadding);     // debug

    // Read the pixel data of the cropped image
    fseek(file, 54, SEEK_SET);              // Move the file pointer to the start of the pixel data
    for (int i = 0; i < croppedHeight; ++i) // Read the pixel data into the allocated memory
    {
        fread(croppedArray + i * cropWidthRGB, 1, cropWidthRGB, file);
        fseek(file, cropPadding, SEEK_CUR); // Skip padding bytes
    }
    fclose(file); // Close the image file
